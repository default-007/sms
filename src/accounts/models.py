...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    def get_assigned_roles_safe(self):
        """
        Get assigned roles with safe cache handling.
        This is a safer version that won't crash if cache fails.
        """
        ...
        try:
            ...
            logger.warning(f"Cache get failed for user {self.id}: {e}")

        # Fallback to database query
        try:
            roles = self._fetch_roles_from_db()

            # Try to cache the result
            try:
                cache.set(cache_key, roles, timeout)
            except Exception as e:
                logger.warning(f"Cache set failed for user {self.id}: {e}")

            return roles
        except Exception as e:
            logger.error(f"Error fetching roles for user {self.id}: {e}")
            return []

    def _fetch_roles_from_db(self):
        """Fetch roles from database."""
        try:
            # Adjust this query based on your actual role model structure
            role_assignments = self.role_assignments.select_related("role").filter(
                is_active=True
            )
            return [assignment.role for assignment in role_assignments]
        except Exception as e:
            logger.error(f"Database query failed for user roles: {e}")
            return []

    def get_assigned_roles(self):
        """
        Backward compatibility method.
        Calls the safe version to prevent crashes.
        """
        return self.get_assigned_roles_safe()

    def clear_user_cache(self):
        """Clear all cache entries for this user."""
        cache_patterns = [
            f"user_roles_{self.id}",
...
# Add this function to handle cache clearing safely
def safe_clear_cache_for_model(model_name, object_id, cache_patterns):
    """
    Safely clear cache patterns for a model instance.
    """
    for pattern in cache_patterns:
        cache_key = pattern.format(id=object_id)
        try:
            cache.delete(cache_key)
        except Exception as e:
            logger.warning(f"Failed to clear cache {cache_key}: {e}")
