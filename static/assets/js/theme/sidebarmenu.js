...
if ((at = "vertical")) {
	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		if (isSidebar.length > 0) {
			...
			var closestNav = elements && elements.closest("nav[class^=sidebar-nav]");
			...
			document
				...
				.forEach(function (item) {
					item.addEventListener("click", function () {
						...
						var targetNav = document.getElementById("menu-right-" + id);
						if (targetNav) {
...
// Wrap this in DOMContentLoaded to ensure DOM is ready
document.addEventListener("DOMContentLoaded", function () {
	var currentURL =
	...
	// Check if the element exists before trying to set attributes
	if (link) {
		...
		console.warn("Element with ID 'get-url' not found in the DOM");
	}
});
