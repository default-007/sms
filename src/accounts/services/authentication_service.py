...
class AuthenticationService:
    ...
    def authenticate_user(
        ...
        try:
            ...
            if user.check_password(password):
                ...
                identifier_type = AuthenticationService._get_identifier_type(identifier)
    ...
    def _find_user_by_identifier(identifier: str) -> Optional[User]:
        ...
        if AuthenticationService._is_email(identifier):
            try:
        ...
        if AuthenticationService._is_phone_number(identifier):
            normalized_phone = AuthenticationService._normalize_phone_number(identifier)
            if normalized_phone:
                try:
        ...
        if AuthenticationService._is_admission_number(identifier):
            try:
                from src.students.models import Student
    ...
    def _get_identifier_type(identifier: str) -> str:
        ...
        if AuthenticationService._is_email(identifier):
            return AuthenticationService.IDENTIFIER_EMAIL
        elif AuthenticationService._is_phone_number(identifier):
            return AuthenticationService.IDENTIFIER_PHONE
        elif AuthenticationService._is_admission_number(identifier):
            return AuthenticationService.IDENTIFIER_ADMISSION
        else:
            return AuthenticationService.IDENTIFIER_USERNAME
    ...
    def _log_failed_attempt(
        ...
        identifier_type = (
            AuthenticationService._get_identifier_type(identifier)
            if identifier
            else "unknown"
        )
    ...
    def _create_session_record(user: User, request) -> None:
        ...
        try:
            ...
            # Update or create session record (without extra_data field)
            UserSession.objects.update_or_create(
                ...
                defaults={
                    ...
                    "is_active": True,
                    # Remove extra_data since it doesn't exist in the model
                },
            )
        except Exception as e:
